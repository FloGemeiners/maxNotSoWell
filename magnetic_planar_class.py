...
Author:
    Florian Meiners - November 25, 2025; Last updated January 2, 2026
"""
...
class DirichletBCMagneticEdge:
    """
    Dirichlet boundary condition for the H(curl) magnetic formulation with Nédélec edge elements.

    Attributes:
    -----------
    edges : np.ndarray
        1D array of global edge indices where the BC is applied.
        Each edge index refers to a row in mesh.edges (node pair).
    value : Optional[ScalarField]
        Function g(x, y) giving the prescribed *tangential* component
        A_t(x, y) = A(x, y) · t̂ in the direction of the global edge
        orientation. If None, homogeneous BC A_t = 0 is used.
    ...
    def values(self, nodes: np.ndarray, edges: np.ndarray) -> np.ndarray:
        ...
        edges : (n_edges, 2) array
            Global edge list (mesh.edges). Each row is (n0, n1) with
            the chosen global orientation.

        Returns:
        -----------
        vals : (len(self.edges),) array
            Prescribed values for the edge DOFs, in the same order
            as self.edges.
        """
        vals = []
...
class Magnetic2D:
    ...
    def _triangle_area_and_grads(p: np.ndarray) -> Tuple[float, np.ndarray]:
        ...
        TODO: Carefully check if this method is actually doing what it is supposed to in terms of correctly calculating
        TODO: gradients. Same holds for duplicates.
        """
        x0, y0 = p[0]
        x1, y1 = p[1]
        x2, y2 = p[2]
...
class Magnetic2DHcurl:
    ...
    def apply_dirichlet(self, bcs: list[DirichletBCMagneticEdge]):
        """
        Applies tangential Dirichlet boundary conditions (so far just A = 0) to the specified edges.

        Parameters:
        -----------
        bcs: list[DirichletEdges]
        ...
        self._free_idx = np.setdiff1d(all_idx, self._fixed_idx, assume_unique=True)
        # TODO: make arbitrary tangential boundary conditions possible
